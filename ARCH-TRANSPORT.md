...
- **Step B/C C++ code discarded (commit `0753217`).** `mctp_core_binding`,
  `mctp_demux_server`, `subprojects/libmctp` removed; `ffs_daemon` and `mctpusbd`
  restored to `bc47414` state.
- **Direction confirmed: Option A1.** Next action: confirm `CONFIG_MCTP_SERIAL` is
  enabled on the aspeed-2700 kernel, then implement PTY + mctp-serial bridge.
- A1 → A2 evolution path unchanged (section 4): `ffs_daemon` and USB header handling
  carry over; only the framing/PTY layer is later replaced by a kernel module.
