> **⚠️ DEPRECATED — 2026-06-25.** This plan describes the original Path A (demux socket:
> mctpusbd as device-side mctp-demux-daemon with libmctp-core). That direction was
> **discarded** (commit `0753217`) after architectural analysis revealed a dual-role
> conflict — see [`ARCH-TRANSPORT.md`](./ARCH-TRANSPORT.md).
>
> **New direction: Option A1** (PTY + `mctp-serial` line discipline → kernel MCTP stack).
> This file is kept for historical reference (PLDM wire-format analysis is still accurate).
> **Do not implement what is described here.**

# Plan — Path A: PLDM over MCTP-USB (mctpusbd as demux server) [DEPRECATED]
